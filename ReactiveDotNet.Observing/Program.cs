...
namespace ReactiveDotNet.Observing
{
	class Program
	{
		...
		private static readonly Queue<char> buffer = new Queue<char>();
		...
		private static void HandleKeysViaManualObservable()
		{
			...
			keyLogger.Subscribe(new ObservingCancellingKeyWatcher("Observer 3", keyLogger, Program.Termination));
			keyLogger.Listen();
		}
